...
Result<NonNullPtr<Slab>> SlabAllocator::get_or_create_slab(size_t size) {
...
Result<void> Slab::initialise(size_t size) {
    ...
    m_initialised = true;
    m_object_size = size;
    auto page_address = TRY(memory_manager.allocate_kernel_heap_page());
...
Result<void> SlabPage::initialise(size_t size) {
    ...
    for (int i = 0; i < m_total_objects; i++) {
        ...
        auto next_object_address = i < m_total_objects ? get_object_address(i + 1).as_address() : 0;
        auto free_object = reinterpret_cast<FreeObject *>(object_address.as_address());
...
Result<void> SlabPage::free(VirtualAddress address) {
    auto free_object = reinterpret_cast<FreeObject *>(address.as_address());
    if (m_free_list == nullptr) {
