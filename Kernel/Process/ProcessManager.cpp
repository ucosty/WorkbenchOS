...
Result<void> ProcessManager::initialise() {
    ...
    m_allocator = TRY_INTO(Error, slab_allocator.get_or_create_slab(sizeof(Process))).as_ptr();
...
Result<Process *> ProcessManager::create_process() {
    auto &memory_manager = MemoryManager::get_instance();
    auto *process = TRY(m_allocator->allocate<Process>());

    const auto stack = TRY_INTO(Error, memory_manager.allocate_kernel_heap_page());
    constexpr StackFrame initial_stack_frame{
        .rip = 0x100000,
        .cs = 0x18 | 3,
        .rsp = 0x100000,
        .ss = 0x20 | 3,
...
//    auto physical_address = TRY(memory_manager.kernel_virtual_to_physical_address(VirtualAddress(&user_text_start)));
    ...
    const auto page_directory = TRY_INTO(Error, memory_manager.create_user_mode_directory());
    process->set_page_directory(page_directory);

    auto stack_physical_address = TRY_INTO(Error, memory_manager.kernel_virtual_to_physical_address(stack));
    ...
    Stack process_stack(user_stack_virtual, Page);
